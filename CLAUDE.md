...
## Restart Docker after every change

Always rebuild and restart the container after any change so the user can test it immediately:

```
docker compose up -d --build
```

The app runs on port 9271. Do not report a task as done until the container is running cleanly.

...
# Update Charlotte to the latest tagged release.
# Run this script from the Charlotte repository root.
#
# Use this script for the most stable experience. Tagged releases have been
# reviewed and tested before publication. To track the bleeding edge instead,
# use update-latest.sh.
...
# ---------------------------------------------------------------------------
echo ""
info "Charlotte — update to latest release"
echo ""
...
if [ -z "$LATEST_TAG" ]; then
    error "No release tags found. Use update-latest.sh instead."
    exit 1
fi

info "Updating to release: ${LATEST_TAG}"
echo ""
...
if [ "$SKIP_TESTS" = true ]; then
    ...
    if ! go test ./internal/... 2>&1; then
        echo ""
        error "Tests failed for release ${LATEST_TAG}."
        info "Returning to main branch..."
        git checkout main
        error "Aborting update. Your installation has not changed."
        exit 1
    fi
...
# ---------------------------------------------------------------------------
# Step 6: Wait for the container to start, then check logs
# ---------------------------------------------------------------------------
...
# Done
# ---------------------------------------------------------------------------
success "Charlotte has been updated to release ${LATEST_TAG}."
